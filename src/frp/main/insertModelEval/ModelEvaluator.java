...
public class ModelEvaluator {
    ...
    private ModelEvaluator(String[] args) throws Exception {
        ...
        // Topology file
        String topologyFileNameBase = CmdLineTools.getRequiredArg(
                CmdLineTools.getName(PROG_ARGS, I_TOP), lstArgs);

        // words mapped to storage nodes data file
        String routeFileName = CmdLineTools.getRequiredArg(
                CmdLineTools.getName(PROG_ARGS, I_ROUTE), lstArgs);
    ...
    public void run(String outputFileName, String topologyFileNameBase,
        ...
        for (ActualRoutePath actPath : actPaths) {
            ...
            if (!prevDataSet.equals(filePostFix)) {
                ...
                pathSets = manager.calculateRoutesFromNodes(null, null,
                        topology, true);
            ...
            outputStream
                    .println(comp.compareStorageNodes(topology,
                            manager.getResetHTL(), actPath.getHTL(), pathSets,
                            actPath));
        }
    }
}
