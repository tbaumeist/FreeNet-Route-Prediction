...
public class AttackNodeAnalysisSingle {
    ...
    public static void main(String[] args) {
        try {
            ...
            // / Arguments
            String appendStr = CmdLineTools.getArg(
                    CmdLineTools.getName(PROG_ARGS, I_APP), lwArgs, "");
            ...
            String nodeCount = CmdLineTools.getRequiredArg(
                    CmdLineTools.getName(PROG_ARGS, I_NODE), lwArgs);

            String peerCount = CmdLineTools.getRequiredArg(
                    CmdLineTools.getName(PROG_ARGS, I_PEER), lwArgs);

            String htlStr = CmdLineTools.getRequiredArg(
                    CmdLineTools.getName(PROG_ARGS, I_HTL), lwArgs);
            int htl = Integer.parseInt(htlStr);

            String maxHTLStr = CmdLineTools.getArg(
                    CmdLineTools.getName(PROG_ARGS, I_MAX_HTL), lwArgs,
                    AttackNodeAnalysisSingle.MAX_HTL_COLUMNS + "");
            int maxHTL = Integer.parseInt(maxHTLStr);

            String dataSetCount = CmdLineTools.getRequiredArg(
                    CmdLineTools.getName(PROG_ARGS, I_DATASET), lwArgs);

            AttackNodeAnalysisSingle s = new AttackNodeAnalysisSingle(maxHTL,
                    topologyFileName);
    ...
    public AttackNodeAnalysisSingle(int totalHTLColumns, String topFileName)
            throws Exception {
